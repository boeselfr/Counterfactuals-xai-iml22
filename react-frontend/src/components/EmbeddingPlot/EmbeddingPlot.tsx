...
//<AxisBottom top={yMax} scale={xScale} stroke='white'/>
    ...
    return (
        <Container fixed>
            <Card elevation={3}>
                <CardContent>
                    <Typography variant="h4" component="div"> UMAP Visualization of Sentence
                        Embeddings </Typography>
                    <Divider/>
                    <Box sx={{my: 3, mx: 2}}>
                        <svg width={width} height={height}>
                            <Group left={margin.left} top={margin.top}>
                                ...
                                <text x="-200" y="-70" transform="rotate(-90)" fontSize={30}
                                      fill='black'>
                                    x2
                                </text>
                                <text x="500" y="500" transform="rotate(0)" fontSize={30}
                                      fill='black'>
                                    x1
                                </text>
                                {data.map((d, idx) => (
                                    <DataPointComponent key={idx} x={xScale(d.X1)}
                                                        y={yScale(d.X2)}
                                                        color={colors(d.gold_label) as string}
                                                        hover={d.sentence2}
                                    />
                                ))}
                            </Group>
